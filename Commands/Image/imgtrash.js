...
const execute = async (client,msg) => {
    ...
    const cd = await User.getImgCd(userid, 'imgtrash', 1)
    ...
    async function print(mediafile) {
        ...
        fs.writeFile(`${folder}/imgtrash.jpg`,mediafile.data,"base64",function (err) {if (err) {console.log(err);}});

        try {
            let bg = await jimp.read('Assets/MemeTemplates/trash.png');
            const image = await jimp.read(`${folder}/imgtrash.jpg`);
            image.resize(309, 304);
            image.blur(5);
            ...
            bg.getBuffer(`image/png`, async (err, buffer) => {
                ...
                const media = new MessageMedia('image/png',base64File , 'trash');
                await msg.reply(media)
                fs.rmSync(folder, { recursive: true, force: true });
                await User.getImgCd(userid, 'imgtrash', 0)
                return;
            });;
    ...
    try {
        ...
        await msg.reply('Just send a pic with *$imgtrash* as caption or quote one.')
    }
