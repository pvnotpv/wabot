...
const execute = async (client, msg, args) => {
    ...
    if(!args.length || args[0].indexOf('@') || !numCheck.test(args[0])) {
        await msg.reply('Usage: *$sendbooba @user*')
        return;
    }
    ...
    if(!usercheck.picperms.boobpics) {
       await msg.reply('Uh oh , the user has disabled *$sendbooba*.')
       return;
    }
    ...
    async function boob() {
        try {
          ...
          await client.sendMessage(user, media, {caption: `@${auser} sent you a boob pic!`})
          await msg.reply(`Done, boob pic sent to @${userid}`)
          return;
    ...
    const cd = await User.getNsfwCd(auser, 'sendboobs', 1)
    ...
    try {
        await boob()
        await User.getNsfwCd(auser, 'sendboobs', 0)
