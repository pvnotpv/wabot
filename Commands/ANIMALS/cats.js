...
const execute = async (client,msg) => {
  ...
  const value = await rClient.get('cats')
  ...
  async function cat() {
    try {
      ...
      await client.sendMessage(msg.from,media, {caption: 'uwu'})
      return;
  ...
  const cd = await User.getAnimalCd(userid, 'cats', 1)
  ...
  try {
    await cat()
    await User.getAnimalCd(userid, 'cats', 0)
